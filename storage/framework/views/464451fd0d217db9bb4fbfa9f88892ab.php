<?php $__env->startSection('header_title', 'Konfirmasi Peminjaman'); ?>
...
<div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2 kpinjam-header">
    <div class="d-flex align-items-center gap-2 flex-wrap">
        ...
        <?php if($permintaan->count() > 0): ?>
            <span style="background:#e74c3c;color:white;border-radius:20px;padding:2px 10px;font-size:12px;font-weight:600">
                <?php echo e($permintaan->count()); ?> menunggu
            </span>
        <?php endif; ?>
    </div>
    <?php if($permintaan->count() > 0): ?>
    <form action="<?php echo e(route('admin.pinjam.konfirmasiSemua')); ?>" method="POST" onsubmit="return confirm('Konfirmasi semua?')">
        <?php echo csrf_field(); ?>
        <button type="submit"
...
<div class="kpinjam-tbl-wrap">
    <table class="table table-hover align-middle mb-0">
        ...
        <tbody>
            <?php $__empty_1 = true; $__currentLoopData = $permintaan; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <tr>
                <td><?php echo e($index + 1); ?></td>
                <td>
                    <div style="font-weight:600;color:#222;font-size:13px"><?php echo e($item->anggota->nama ?? '-'); ?></div>
                    <div style="font-size:11px;color:#888"><?php echo e($item->anggota->email ?? '-'); ?></div>
                </td>
                <td>
                    <div style="display:flex; align-items:center; gap:10px">
                        <?php if($item->buku->sampul): ?>
                            <img src="<?php echo e(asset($item->buku->sampul)); ?>" style="width:35px;height:48px;object-fit:cover;border-radius:5px;flex-shrink:0">
                        <?php else: ?>
                        ...
                        <?php endif; ?>
                        <div>
                            <div style="font-weight:600;color:#222;font-size:13px"><?php echo e($item->buku->judul ?? '-'); ?></div>
                            <div style="display: flex; align-items: center; gap: 8px;">
                                <span style="font-size:11px;color:#888"><?php echo e($item->buku->pengarang ?? '-'); ?></span>
                ...
                <td style="font-size:13px"><?php echo e($item->tanggal_pinjam); ?></td>
                ...
                <td style="white-space:nowrap">
                    <form action="<?php echo e(route('admin.pinjam.konfirmasi', $item->id)); ?>" method="POST" class="d-inline"
                        onsubmit="return confirm('Konfirmasi peminjaman buku <?php echo e(addslashes($item->buku->judul ?? '')); ?> oleh <?php echo e(addslashes($item->anggota->nama ?? '')); ?>?')">
                        <?php echo csrf_field(); ?>
                        <button type="submit"
                    ...
                    <form action="<?php echo e(route('admin.pinjam.tolak', $item->id)); ?>" method="POST" class="d-inline"
    ...
    <div class="kpm-card">
        <div class="kpm-card-top">
            <div class="kpm-card-user">
                <div class="kpm-avatar"><?php echo e(strtoupper(substr($item->anggota->nama ?? '?', 0, 1))); ?></div>
                <div>
                    <div class="kpm-user-name"><?php echo e($item->anggota->nama ?? '-'); ?></div>
                    <div class="kpm-user-email"><?php echo e($item->anggota->email ?? '-'); ?></div>
                </div>
            </div>
            <span class="kpm-durasi"><?php echo e($durasi); ?> hari</span>
        </div>

        <div class="kpm-book-wrap">
            <?php if($item->buku->sampul): ?>
                <img src="<?php echo e(asset($item->buku->sampul)); ?>" class="kpm-book-img">
            <?php else: ?>
            ...
            <?php endif; ?>
            <div>
                <div class="kpm-book-title"><?php echo e($item->buku->judul ?? '-'); ?></div>
                <div class="kpm-book-info">
                    <span class="kpm-book-author"><?php echo e($item->buku->pengarang ?? '-'); ?></span>
                    <?php if($item->eksemplar): ?>
                        <span class="kpm-badge-kode"><?php echo e($item->eksemplar->kode_buku); ?></span>
                    <?php endif; ?>
                    <span class="kpm-badge-tersedia">Tersedia: <?php echo e($item->buku->eksemplarTersedia()->count()); ?></span>
                </div>
            </div>
        </div>
        ...
        <div class="kpm-card-actions">
            <form action="<?php echo e(route('admin.pinjam.konfirmasi', $item->id)); ?>" method="POST"
                onsubmit="return confirm('Konfirmasi peminjaman buku <?php echo e(addslashes($item->buku->judul ?? '')); ?> oleh <?php echo e(addslashes($item->anggota->nama ?? '')); ?>?')">
                <?php echo csrf_field(); ?>
            ...
            <form action="<?php echo e(route('admin.pinjam.tolak', $item->id)); ?>" method="POST"
                onsubmit="return confirm('Tolak permintaan peminjaman ini?')">
                <?php echo csrf_field(); ?>
                <?php echo method_field('DELETE'); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\PerpustakaanDigital\resources\views\admin\konfirmasi-pinjam.blade.php ENDPATH**/ ?>
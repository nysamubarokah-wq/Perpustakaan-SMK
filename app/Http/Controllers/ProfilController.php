  ...
  public function index()
{
    ...
    // 2. Ambil ID Anggota berdasarkan email user yang sedang login
    $anggota = \App\Models\Anggota::where('email', $user->email)->first();
    $anggotaId = $anggota ? $anggota->id : 0;

    // 3. Ambil data peminjaman menggunakan ID yang sudah didapat
   $riwayat = \App\Models\Peminjaman::with('buku')
    ...
    // 4. Hitung total favorit user
    $totalFavorit = \App\Models\Favorit::where('user_id', $user->id)->count();

    // 5. Kirim semua data ke view
    return view('profil.index', compact('riwayat', 'totalDendaBelumBayar', 'user', 'totalFavorit'));
}

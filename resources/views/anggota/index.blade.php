    ...
    <form method="GET" action="{{ route('anggota.index') }}">
        ...
        <table class="table table-hover">
            <thead style="background:#f8f9fa">
                <tr>
                    <th>#</th><th>Nama</th><th>NIS</th><th>Email</th><th>No. Telepon</th><th>Alamat</th><th>Tgl Daftar</th><th>Aksi</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $item->nama }}</td>
                    <td>{{ \App\Models\User::where('email', $item->email)->first()?->nis ?? '-' }}</td>
                    <td>{{ $item->email }}</td>
                    ...
                    <td style="white-space: nowrap;">
                        ...
                        <a href="{{ route('anggota.edit', $item->id) }}" class="btn btn-sm btn-warning"><i class="bi bi-pencil"></i></a>
                        <form action="{{ route('anggota.destroy', $item->id) }}" method="POST" class="d-inline">
                            @csrf @method('DELETE')
                            <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Yakin hapus?')"><i class="bi bi-trash"></i></button>
                        </form>
                    </td>
                </tr>
                @empty
        ...
        <table class="table table-hover">
            ...
            <tbody>
                ...
                <tr>
                   ...
                   <td>
    {{ $item->nama }}
    @if(\App\Models\User::where('email', $item->email)->first()?->id === auth()->id())
...
<td>{{ $item->email }}</td>
                    ...
                    <td>{{ $item->tanggal_daftar }}</td>
                    <td style="white-space: nowrap;">
                        {{-- INI FORM YANG BENAR UNTUK JADIKAN SISWA --}}
                ...
                @empty
                <tr><td colspan="8" class="text-center text-muted py-4">Belum ada data admin</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>

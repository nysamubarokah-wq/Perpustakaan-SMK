...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .badge-baru { position: absolute; top: 10px; left: 10px; background: linear-gradient(135deg, #1a6e35, #27ae60); color: white; padding: 4px 12px; border-radius: 20px; font-size: 11px; font-weight: 700; z-index: 5; }
                ...
                <ul class="navbar-nav flex-row gap-1 mb-0">
                    ...
                    <li class="nav-item">
                        ...
                        <div class="floating-dropdown" id="genreDropdown">
                            <h6>Pilih Genre</h6>
                            <a href="{{ route('koleksi.terbaru', $penerbit ? ['penerbit' => $penerbit] : []) }}" class="genre-list-item"><i class="bi bi-grid-fill"></i> Semua Buku</a>
                            ...
                            <a href="{{ route('koleksi.terbaru', array_filter(['genre' => $g['nama'], 'penerbit' => $penerbit])) }}" class="genre-list-item"><i class="bi {{ $g['icon'] }}"></i> {{ $g['nama'] }}</a>
                            @endforeach
                        </div>
                    </li>
                    <li class="nav-item">
...
<div class="container" style="padding-top:24px">
    <div class="penerbit-scroll">
        ...
        <a href="{{ route('koleksi.terbaru', array_filter(['penerbit' => $p, 'genre' => $genre])) }}" class="penerbit-chip {{ $penerbit === $p ? 'active' : '' }}">{{ $p }}</a>
        @endforeach
    </div>
</div>
@endif
...
function performSearch() {
    ...
    results.forEach(function(item, index) {
        ...
        var html = '<div class="col-6 col-md-4 col-lg-3">' +
            ...
            '<p class="book-card-meta"><i class="bi bi-building"></i> ' + (item.penerbit || '-') + '</p>' +
            genreHtml +
            '<div style="margin-top:auto;margin-bottom:10px"><span class="status-badge ' + statusClass + '">' + statusText + '</span></div>' +
            btnHtml +
            '</div></div></div>';

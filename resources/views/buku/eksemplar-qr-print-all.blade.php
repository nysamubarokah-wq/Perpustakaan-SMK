...
<head>
    <meta charset="UTF-8">
    <title>Cetak QR Eksemplar{{ $buku ? ' - '.$buku->judul : '' }}</title>
    <style>
...
<body>
    <div class="no-print" style="text-align:center;margin-bottom:20px">
        ...
        <a href="{{ $buku ? route('buku.show', $buku->id) : route('buku.index') }}" style="display:inline-block;margin-left:10px;padding:10px 24px;background:#f0f0f0;color:#555;border:none;border-radius:8px;font-size:14px;font-weight:600;text-decoration:none">
            Kembali
        </a>
    </div>
    <div class="header">
        <h1>{{ $buku ? 'QR Code Eksemplar - '.$buku->judul : 'Semua QR Code Eksemplar' }}</h1>
        <p>Total: {{ count($items) }} eksemplar &mdash; Dicetak: {{ date('d M Y H:i') }}</p>
    </div>
    <div class="grid">
        @foreach($items as $item)
        <div class="card">
            <h3>{{ $item['eksemplar']->buku->judul ?? ($buku->judul ?? '-') }}</h3>
            <div class="kode">{{ $item['eksemplar']->kode_buku }}</div>

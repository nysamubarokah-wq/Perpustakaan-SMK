...
<div class="card-admin" style="margin-bottom:20px">
    <div class="card-admin-body">
        <form action="{{ route('buku.eksemplar.tambah', $buku->id) }}" method="POST" style="display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap">
            @csrf
            <div style="flex:1;min-width:200px">
                <label class="form-label fw-600" style="font-size:13px">Tambah Eksemplar Baru</label>
...
<div class="card-admin">
    <x-admin-card-header title="Daftar Eksemplar" icon="bi bi-collection">
        <x-slot:action>
            <a href="{{ route('buku.eksemplar.cetakQr', $buku->id) }}" target="_blank"
                ...
                <div style="background:white;border:1px solid #e5e7eb;border-radius:12px;padding:15px;display:flex;gap:12px;align-items:flex-start">
                    ...
                    <div style="flex-shrink:0">
                        @if($eksemplar->qr_exists)
                            <img src="{{ asset($eksemplar->qrcode_path) }}" style="width:80px;height:80px;border-radius:8px;border:1px solid #eee">
                        @else
                            <div style="width:80px;height:80px;background:#f3f4f6;border-radius:8px;display:flex;align-items:center;justify-content:center">
                                <i class="bi bi-qr-code" style="font-size:24px;color:#ccc"></i>
                            </div>
                        @endif
                    </div>
                    {{-- Info --}}
                    <div style="flex:1;min-width:0">
                        ...
                        <div style="margin-top:8px;display:flex;gap:5px;flex-wrap:wrap">
                            <a href="{{ route('eksemplar.qrcodeDownload', $eksemplar->id) }}"
                               style="padding:4px 10px;background:#dbeafe;color:#2563eb;border:none;border-radius:6px;font-size:10px;font-weight:600;text-decoration:none;display:inline-flex;align-items:center;gap:3px">
                                <i class="bi bi-download"></i> Download
                            </a>
                            <a href="{{ route('eksemplar.qrcodePrint', $eksemplar->id) }}" target="_blank"
                               style="padding:4px 10px;background:#fef3c7;color:#92400e;border:none;border-radius:6px;font-size:10px;font-weight:600;text-decoration:none;display:inline-flex;align-items:center;gap:3px">

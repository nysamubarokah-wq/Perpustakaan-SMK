...
<head>
    ...
    <title>{{ $ebook->judul }} - Perpustakaan SMK Maarif</title>
...
<nav class="navbar">
    <div class="container-fluid px-4">
        <div class="d-flex align-items-center justify-content-between w-100">
            ...
            <a href="{{ route('ebook.index') }}" style="color:#1a6e35;text-decoration:none;font-size:14px;font-weight:500">
...
<div class="main-container">
    ...
    @if(session('error'))
        <div class="alert alert-danger mb-3" style="border-radius:12px">{{ session('error') }}</div>
    @endif
  ...
  <div class="detail-card">
    ...
    <div class="left-section">
        @if($ebook->cover)
            <img src="{{ asset($ebook->cover) }}" class="sampul-img" alt="{{ $ebook->judul }}">
        @else
    ...
    <div class="right-section">
        ...
        <h1 class="judul">{{ $ebook->judul }}</h1>
        <p class="penulis"><i class="bi bi-person"></i> {{ $ebook->penulis }}</p>

        {{-- Sinopsis dipindahkan ke sini --}}
        @if($ebook->sinopsis)
        ...
        <div style="background:#f8f9fa;border-radius:12px;padding:15px 18px;margin-bottom:20px" class="info-box">
            ...
            <div style="font-size:18px;font-weight:700;color:#333" class="info-value">🪙 {{ auth()->user()->coin ?? 0 }} Coin</div>
        </div>

        {{-- Tombol aksi --}}
        @if($bisaDiakses)
            <a href="{{ route('ebook.baca', $ebook->id) }}" class="btn-baca">
                <i class="bi bi-book-open"></i> Baca Sekarang
            </a>
        @else
            {{-- Bagian logika tombol lainnya tetap sama --}}
            @if($ebook->is_vip)
            ...
            @elseif($ebook->harga_koin > 0)
                <div style="background:#f0fdf4;border-radius:12px;padding:15px;text-align:center;margin-bottom:15px" class="alert-koin">
                    <div style="font-size:13px;color:#888;margin-bottom:5px">Butuh <strong>🪙 {{ $ebook->harga_koin }} koin</strong> untuk membaca</div>
                    @if((auth()->user()->coin ?? 0) < $ebook->harga_koin)
                        <div style="font-weight:700;color:#e74c3c;font-size:13px">Koin kamu tidak cukup</div>
                    @endif
                </div>
                @if((auth()->user()->coin ?? 0) >= $ebook->harga_koin)
                    <form action="{{ route('ebook.beli', $ebook->id) }}" method="POST">
                        @csrf
                        <button type="submit" class="btn-baca">
                            🪙 Baca dengan {{ $ebook->harga_koin }} Koin
                        </button>
                    </form>
                @else
